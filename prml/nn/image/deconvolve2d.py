    ...
    def _forward(self, x, y):
        if self.shape is None:
            shape = (
                ...
                + tuple(
                    s * (imlen - 1) + klen
                    for s, imlen, klen in zip(self.stride, x.shape[1:], self.kernel_size)
                )
                + (self.out_ch,)
            )
        else:
        ...
        output = patch2img(
            patch_flat.reshape(*patch_flat.shape[:3], *self.kernel_size, -1), self.stride, shape
        )
        output = output[
            :,
...
class Deconvolve2d(Network):
    ...
    def __call__(self, x):
        func = Deconvolve2dFunction(
            self.kernel_size, self.out_ch, self.stride, self.pad, self.shape
        )
